...
bool Renderer::Init(const wchar_t* a_windowTitle)
{
	// Create application window
	ImGui_ImplWin32_EnableDpiAwareness();
	wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"Main", nullptr };
	HICON icon = LoadIconA(GetModuleHandle(nullptr), "IDI_ICON1");
	wc.hIcon = icon;
	wc.hIconSm = icon;
	::RegisterClassExW(&wc);
	mainHWND = ::CreateWindowW(wc.lpszClassName, a_windowTitle, WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, nullptr, nullptr, wc.hInstance, nullptr);
...
LRESULT WINAPI WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		if (wParam != SIZE_MINIMIZED) {
			...
			renderer.resizeWidth = LOWORD(lParam);
			renderer.resizeHeight = HIWORD(lParam);
		}
		return 0;
	case WM_SYSCOMMAND:
